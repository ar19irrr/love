...
def is_admin_blocked(user_id: int) -> bool:
    """بررسی اینکه کاربر توسط ادمین بلاک شده (فقط برای خودش)"""
...
def chat_keyboard(other_user: int):
    """کیبورد شیشه‌ای داخل چت - مثل تصویر"""
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("📸 درخواست عکس", callback_data=f"ph_{other_user}")],
...
def get_interests_keyboard(selected=None):
    ...
    for i in range(0, len(interests_list), 3):
        ...
        for item in interests_list[i:i+3]:
            if item in selected:
                row.append(InlineKeyboardButton(f"✅ {item}", callback_data=f"int_{item}"))
            else:
                row.append(InlineKeyboardButton(f"⬜ {item}", callback_data=f"int_{item}"))
        keyboard.append(row)
    
    keyboard.append([InlineKeyboardButton("✅ تموم شد", callback_data="int_done")])
    return InlineKeyboardMarkup(keyboard)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("👨 مرد", callback_data="g_male")],
            [InlineKeyboardButton("👩 زن", callback_data="g_female")],
            [InlineKeyboardButton("🧑 سایر", callback_data="g_other")]
        ])
    )
    return GENDER
...
async def age_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "🎯 هدف؟",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("💍 ازدواج", callback_data="p_marriage")],
            [InlineKeyboardButton("💑 دوستی", callback_data="p_relationship")],
            [InlineKeyboardButton("🎭 رفاقت", callback_data="p_friendship")],
            [InlineKeyboardButton("🤷 نمیدونم", callback_data="p_unknown")]
        ])
    )
    return PURPOSE
...
async def purpose_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    purpose_map = {"p_marriage": "ازدواج", "p_relationship": "دوستی", "p_friendship": "رفاقت", "p_unknown": "نمیدونم"}
    context.user_data['purpose'] = purpose_map[query.data]
    
    await query.edit_message_text("🏙️ شهرت؟", reply_markup=None)
    return CITY

async def city_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data['city'] = update.message.text
    await update.message.reply_text("📏 حداقل سن طرف:", reply_markup=None)
    return AGE_MIN
...
async def age_min_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("📏 حداکثر سن:", reply_markup=None)
    return AGE_MAX
...
async def interests_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "int_done":
        if not context.user_data['interests']:
            await query.edit_message_text("❌ حداقل یک مورد!", reply_markup=None)
            await show_interests(update, context)
            return INTERESTS
        ...
        await query.edit_message_text(
            "💼 وضعیت؟",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🎓 دانشجو", callback_data="j_student")],
                [InlineKeyboardButton("💼 شاغل", callback_data="j_employed")],
                [InlineKeyboardButton("🔍 جویای کار", callback_data="j_seeking")],
                [InlineKeyboardButton("🏠 خانه‌دار", callback_data="j_home")]
            ])
        )
        return JOB_STATUS
    
    interest = query.data.replace("int_", "")
    if interest in context.user_data['interests']:
        ...
        if len(context.user_data['interests']) >= 5:
            await query.edit_message_text("❌ حداکثر ۵!", reply_markup=None)
            await show_interests(update, context)
            return INTERESTS
...
async def job_status_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    job_map = {"j_student": "دانشجو", "j_employed": "شاغل", "j_seeking": "جویای کار", "j_home": "خانه‌دار"}
    context.user_data['job_status'] = job_map[query.data]
    
    await query.edit_message_text(
        "📝 توضیحات (۲۰۰ کاراکتر)\nیا رد شدن:",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⏭️ رد شدن", callback_data="desc_skip")]])
    )
    return DESCRIPTION
...
async def description_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "📸 عکس پروفایل:\n(رد شدن = عکس تلگرام)",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⏭️ رد شدن", callback_data="photo_skip")]])
    )
    return PHOTO
...
async def handle_photo_upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.photo:
        ...
        await update.message.reply_text("❌ عکس بفرست!", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⏭️ رد شدن", callback_data="photo_skip")]]))
        return PHOTO
...
async def show_privacy_settings(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton(f"{age_status} نمایش سن", callback_data="pr_age")],
        [InlineKeyboardButton(f"{city_status} نمایش شهر", callback_data="pr_city")],
        [InlineKeyboardButton(f"🌍 نمایش به: {vis_text}", callback_data="pr_vis")],
        [InlineKeyboardButton("✅ تایید", callback_data="pr_done")]
    ]
...
async def search(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # کاربران بلاک شده توسط ادمین - فقط برای این کاربر خاص
...
async def show_candidate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if index >= len(candidates):
        await query.edit_message_text("🎯 پایان جستجو!", reply_markup=main_menu_keyboard())
        return
    ...
    keyboard = [
        [InlineKeyboardButton("👍", callback_data=f"lk_{user_dict['user_id']}"),
         InlineKeyboardButton("👎", callback_data=f"dl_{user_dict['user_id']}"),
         InlineKeyboardButton("❓", callback_data=f"mo_{user_dict['user_id']}")],
        [InlineKeyboardButton("⏩ بعدی", callback_data="next_cand")]
    ]
...
async def candidate_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin_blocked(target_id):
        await query.edit_message_text("❌ این کاربر مسدود شده!", reply_markup=main_menu_keyboard())
        return
    ...
    if action == "lk":
            ...
            if not existing:
                ...
                try:
                    await context.bot.send_message(
                        ...
                        reply_markup=InlineKeyboardMarkup([
                            [InlineKeyboardButton("👀 مشاهده", callback_data=f"vw_{user_id}")],
                            [InlineKeyboardButton("✅ تایید", callback_data=f"ac_{user_id}")],
                            [InlineKeyboardButton("❌ رد", callback_data=f"rj_{user_id}")]
                        ])
                    )
                except:
                    pass
        ...
        if target_user:
            ...
            await query.edit_message_text(message, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 برگشت", callback_data=f"bc_{target_id}")]]))
...
async def show_chats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for chat in chats:
        ...
        if other and not is_admin_blocked(other_user):
            ...
            keyboard.append([InlineKeyboardButton(f"💬 {other['gender']} {other['age']}ساله{last_msg}", callback_data=f"sw_{chat['id']}")])
...
async def blocked_users_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for item in blocked:
        ...
        keyboard.append([InlineKeyboardButton(f"🔓 آنبلاک {item['blocked_id']}", callback_data=f"ub_{item['blocked_id']}")])
...
async def unblock_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        db.execute("DELETE FROM rejected WHERE user_id=? AND rejected_user_id=?", (user_id, target_id))
        
        await query.edit_message_text(f"✅ {target_id} آنبلاک شد!", reply_markup=main_menu_keyboard())
        ...
        if ADMIN_ID:
            try:
                await context.bot.send_message(
                    ...
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("🚫 بلاک", callback_data=f"ab_{user_id}")],
                        [InlineKeyboardButton("📋 گزارش‌ها", callback_data="admin_reports")]
                    ])
                )
            except:
...
async def admin_unblock_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for user_id in blocked:
        ...
        if user:
            message += f"👤 {user_id}\n"
            keyboard.append([InlineKeyboardButton(f"✅ آنبلاک {user_id}", callback_data=f"aub_{user_id}")])
...
async def admin_reports(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for r in reports:
        ...
        keyboard.append([
            InlineKeyboardButton(f"🚫 بلاک", callback_data=f"abr_{r['reported_id']}_{r['id']}"),
            InlineKeyboardButton(f"✅ بستن", callback_data=f"acr_{r['id']}")
        ])
...
async def admin_block_from_report(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(f"✅ {user_id} بلاک و گزارش بسته شد!", reply_markup=admin_panel_keyboard())
...
async def admin_close_report(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(f"✅ گزارش {report_id} بسته شد!", reply_markup=admin_panel_keyboard())
...
async def view_requester(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton("✅ تایید", callback_data=f"ac_{requester_id}")],
        [InlineKeyboardButton("❌ رد", callback_data=f"rj_{requester_id}")],
        [InlineKeyboardButton("🔙 برگشت", callback_data="back_to_menu")]
    ]
...
async def edit_profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    info += f"🎨 {', '.join(interests) if interests else '❌'}\n"
    info += f"💼 {user['job_status']}\n"
    info += f"📸 {'✅' if user['photo_file_id'] else '❌'}\n\n"
    info += "کدوم بخش رو ویرایش کنی؟"
    
    keyboard = [
        [InlineKeyboardButton("👤 جنسیت", callback_data="ed_g"), InlineKeyboardButton("📅 سن", callback_data="ed_a")],
        [InlineKeyboardButton("🎯 هدف", callback_data="ed_p"), InlineKeyboardButton("🏙️ شهر", callback_data="ed_c")],
        [InlineKeyboardButton("🎨 علایق", callback_data="ed_i"), InlineKeyboardButton("💼 وضعیت", callback_data="ed_j")],
        [InlineKeyboardButton("📝 توضیحات", callback_data="ed_d")],
        [InlineKeyboardButton("📸 عکس", callback_data="ed_ph")],
        [InlineKeyboardButton("🔙 برگشت", callback_data="back_to_menu")]
    ]
...
async def edit_profile_field(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if field == "g":
        await query.edit_message_text(
            "جنسیت جدید:",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("👨 مرد", callback_data="up_g_male")],
                [InlineKeyboardButton("👩 زن", callback_data="up_g_female")],
                [InlineKeyboardButton("🧑 سایر", callback_data="up_g_other")],
        ...
        await query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("💍 ازدواج", callback_data="up_p_marriage")],
                [InlineKeyboardButton("💑 دوستی", callback_data="up_p_relationship")],
                [InlineKeyboardButton("🎭 رفاقت", callback_data="up_p_friendship")],
                [InlineKeyboardButton("🤷 نمیدونم", callback_data="up_p_unknown")],
                [InlineKeyboardButton("🔙 برگشت", callback_data="edit_profile")]
            ])
        )
    elif field == "c":
        ...
        await query.edit_message_text("🎨 علایق جدید (حداکثر ۵):", reply_markup=get_interests_keyboard(json.loads(user['interests']) if user['interests'] else []))
        ...
        await query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🎓 دانشجو", callback_data="up_j_student")],
                [InlineKeyboardButton("💼 شاغل", callback_data="up_j_employed")],
                [InlineKeyboardButton("🔍 جویای کار", callback_data="up_j_seeking")],
                [InlineKeyboardButton("🏠 خانه‌دار", callback_data="up_j_home")],
                [InlineKeyboardButton("🔙 برگشت", callback_data="edit_profile")]
            ])
        )
    elif field == "d":
...
async def update_profile_field(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("g_"):
        ...
        await edit_interests_selection(update, context)

async def edit_interests_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    
    user_id = update.effective_user.id
    interest = query.data[3:]
...
async def handle_profile_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if field == 'age':
        ...
        # اینجا برای علایق از طریق کیبورد مدیریت میشه
        pass
...
async def privacy_settings(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton(f"{age_status} سن", callback_data="pr_age"),
         InlineKeyboardButton(f"{city_status} شهر", callback_data="pr_city")],
        [InlineKeyboardButton(f"🌍 نمایش به: {vis_text}", callback_data="pr_vis")],
        [InlineKeyboardButton("🔙 برگشت", callback_data="back_to_menu")]
    ]
...
async def my_requests(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for req in received[:5]:
        ...
        if from_user and not is_admin_blocked(from_user['user_id']):
            ...
            keyboard.append([InlineKeyboardButton("👀 مشاهده", callback_data=f"vw_{from_user['user_id']}")])
...
def main():
    try:
        ...
        conv_handler = ConversationHandler(
            ...
            states={
                GENDER: [CallbackQueryHandler(gender_selection, pattern='^g_')],
                AGE: [MessageHandler(filters.TEXT & ~filters.COMMAND, age_input)],
                PURPOSE: [CallbackQueryHandler(purpose_selection, pattern='^p_')],
                ...
                INTERESTS: [CallbackQueryHandler(interests_selection, pattern='^int_')],
                JOB_STATUS: [CallbackQueryHandler(job_status_selection, pattern='^j_')],
                DESCRIPTION: [
                    MessageHandler(filters.TEXT & ~filters.COMMAND, description_input),
                    CallbackQueryHandler(description_skip, pattern='^desc_skip$')
                ],
                PHOTO: [
                    MessageHandler(filters.PHOTO, handle_photo_upload),
                    CallbackQueryHandler(skip_photo, pattern='^photo_skip$'),
                    CallbackQueryHandler(photo_done, pattern='^photo_done$')
                ],
                PRIVACY: [CallbackQueryHandler(privacy_selection, pattern='^pr_')],
            },
        ...
        application.add_handler(CallbackQueryHandler(unblock_user, pattern='^ub_'))
        
        # چت‌ها
        application.add_handler(CallbackQueryHandler(show_chats, pattern='^show_chats$'))
        application.add_handler(CallbackQueryHandler(switch_chat, pattern='^sw_'))
        
        # ویرایش
        application.add_handler(CallbackQueryHandler(edit_profile_field, pattern='^ed_'))
        application.add_handler(CallbackQueryHandler(update_profile_field, pattern='^up_'))
        ...
        # جستجو و درخواست
        application.add_handler(CallbackQueryHandler(candidate_action, pattern='^(lk|dl|mo)_'))
        application.add_handler(CallbackQueryHandler(show_candidate, pattern='^next_cand$'))
        application.add_handler(CallbackQueryHandler(back_to_candidate, pattern='^bc_'))
        application.add_handler(CallbackQueryHandler(view_requester, pattern='^vw_'))
        application.add_handler(CallbackQueryHandler(handle_request, pattern='^(ac|rj)_'))
        
        # چت
        application.add_handler(CallbackQueryHandler(start_chat, pattern='^chat_'))
        application.add_handler(CallbackQueryHandler(request_photo, pattern='^ph_'))
        ...
        application.add_handler(CallbackQueryHandler(admin_block_user, pattern='^admin_block$'))
        application.add_handler(CallbackQueryHandler(admin_unblock_user, pattern='^admin_unblock$'))
        application.add_handler(CallbackQueryHandler(admin_do_unblock, pattern='^aub_'))
        application.add_handler(CallbackQueryHandler(admin_reports, pattern='^admin_reports$'))
        application.add_handler(CallbackQueryHandler(admin_block_from_report, pattern='^abr_'))
        application.add_handler(CallbackQueryHandler(admin_close_report, pattern='^acr_'))
        application.add_handler(CallbackQueryHandler(admin_stats, pattern='^admin_stats$'))
        
        # حریم خصوصی
        application.add_handler(CallbackQueryHandler(privacy_toggle, pattern='^pr_'))
